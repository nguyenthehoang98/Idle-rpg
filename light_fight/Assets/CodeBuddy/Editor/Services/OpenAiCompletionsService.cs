...
// Type: CodeBuddy.Services.OpenAiCompletionsService
...
{
  internal class OpenAiCompletionsService : IAiService
  {
    private static OpenAiCompletionsService instance;
    ...
    public static OpenAiCompletionsService Instance
    {
      get
      {
        if (OpenAiCompletionsService.instance == null)
          OpenAiCompletionsService.instance = new OpenAiCompletionsService();
        OpenAiCompletionsService.instance.Init();
        return OpenAiCompletionsService.instance;
      }
    }
      ...
      Action<string> onDataReceived)
    {
      ...
      else
      {
        ...
        StringContent stringContent = new StringContent(JsonConvert.SerializeObject((object) new ChatCompletionRequest()
        {
          ...
          temperature = (this.settings.Model.StartsWith("o") ? 1f : this.settings.Temperature),
          stream = true,
          tools = ToolManager.Instance.GetToolsJson((IAiService) this)
        }), Encoding.UTF8, "application/json");
        HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, this.baseUrl + "v1/chat/completions")
        {
