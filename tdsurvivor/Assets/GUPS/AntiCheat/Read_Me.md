...
AntiCheat has a centralized configuration which applies global settings at runtime and in the editor. You can find it at 'Edit -> Project settings -> GuardingPearSoftware -> AntiCheat'. AntiCheat has platform settings for Android, iOS and Desktop detectors. These settings work in combination with the platform-specific Monitors and Detectors, check out the chapters 'Protect Android', 'Protect iOS' and 'Protect Desktop'.
...
## Step 9 - Protect Desktop

Desktop builds are common targets for debuggers, mod loaders, injected libraries, and virtual machines. AntiCheat Pro includes desktop detectors for Windows, macOS and Linux standalone players so you can identify these environments and decide how your game should react.

### Debugger Detection

Debuggers let attackers pause your game, inspect memory, change values, and step through your code. AntiCheat can detect managed debuggers, user-mode debuggers, remote debugger attachment and, on Windows, kernel-mode debugger evidence.
...
The 'DebuggerDetector' does not require a monitor. It can scan once on Start, on a configurable interval, or when 'ManualScan()' is invoked. On Windows it combines managed debugger checks with native process and kernel-debugger probes. On macOS and Linux it uses platform-specific process tracing checks for user-mode debugger detection.

### Mod Loader Detection

Mod loaders such as BepInEx, MelonLoader, UnityDoorstop, HarmonyLib and MonoMod can load arbitrary code into your game process. Since these loaders often attach before the C# runtime starts, AntiCheat focuses on detecting their presence and forwarding the detection to your reaction flow.
...
The 'ModLoaderDetector' does not require a monitor. It checks loaded managed assemblies, suspicious native modules and mod-loader files or folders next to the game executable. Managed assembly and file-system checks run on Windows, macOS and Linux. Native module checks are currently implemented for Windows.

### Virtual Environment Detection

Virtual machines such as VMware, VirtualBox, Hyper-V, QEMU/KVM and Parallels can make memory inspection, game-state snapshots and automation easier for attackers. AntiCheat can detect common virtualized hosts so you can flag or restrict suspicious sessions.
...
The 'VirtualEnvironmentDetector' does not require a monitor. It checks network adapter MAC OUIs, BIOS or DMI strings and hypervisor evidence. The MAC OUI scan is cross-platform, BIOS/DMI checks use platform-specific probes, and hypervisor-bit detection is currently effective on Linux.

> [!NOTE]
> Virtual environment detection can produce more false positives than other desktop detectors. Developers, QA teams, cloud-streaming providers, corporate laptops and security-conscious players may legitimately run inside a virtual environment. Consider a softer reaction, such as flagging the session or reducing trust, instead of immediately closing the game.

**Project Settings**

Open 'Edit -> Project Settings -> GuardingPearSoftware -> AntiCheat' and switch to the 'Desktop' tab. There you can enable development-build detection with 'Desktop_Enable_Development' and configure the individual debugger, mod-loader and virtual-machine detector switches and lists.

Add the desktop detectors you need to your 'AntiCheat-Monitor' (more details are provided within each detector's documentation). To react to detection look in the last chapter 'React To Detected Cheating'.

## Step 10 - Protect Android
...
iOS apps are generally protected by Apple's app sandbox and distribution model, but jailbroken devices can bypass those restrictions. Jailbreaks can allow code injection, filesystem access outside the sandbox, binary patching, and reverse-engineering tools such as Frida or Cycript.

### Jailbreak Detection

AntiCheat introduces an iOS jailbreak detector to validate whether the app is running on a jailbroken device. It helps detect jailbreak artefacts and runtime manipulation frameworks so you can react before trusting the device or player session.

**Detector**

| Detector - iOS        | Free | Pro |
| --------------------- | ---- | --- |
| IOSJailbreakDetector  |      | +   |

The 'IOSJailbreakDetector' does not require a monitor. It can scan once on Start, on a configurable interval, or when 'ManualScan()' is invoked. The detector delegates to a native iOS probe and can check jailbreak URL schemes, suspicious filesystem paths, sandbox violations, DYLD injection variables, loaded tweak dylibs and optional 'fork()' support.

**Project Settings**

Open 'Edit -> Project Settings -> GuardingPearSoftware -> AntiCheat' and switch to the 'iOS' tab. There you can enable or disable individual jailbreak probes, add custom suspicious paths, and add custom dylib name fragments to detect additional tweak frameworks.

The URL-scheme probe requires matching entries in the generated iOS 'Info.plist'. AntiCheat's iOS build post-processor adds those entries automatically when the URL-scheme probe is enabled.

**Native Plugin**

For real device scans, the native iOS jailbreak plugin must be present in 'Assets/GUPS/AntiCheat/Plugins/iOS/'. Without the plugin, the detector fails gracefully and logs a warning instead of crashing. Add the 'IOSJailbreakDetector' to your 'AntiCheat-Monitor' (more details are provided within the detector's documentation). To react to detection look in the last chapter 'React To Detected Cheating'.

## Step 12 - React To Detected Cheating
