...
namespace _Game.GamePlay.Model
{
    ...
    {
        public void FindTarget(FindTargetType type, int totalQuery, Vector2 center, Vector2 pivot, float radius, Func<int, float2, bool> funcFilterEntity,
            out QueryResult result)
        {
                ...
                switch (type)
                {
                    case FindTargetType.Nearest:
                        ...
                        if (funcFilterEntity(entity, data.position))
                        {
                            ...
                            candidates.Add((dsq, entity, data.position));
                        }
            ...
            {
                result.Results.Add(new QueryEntityData(candidates[i].entity, candidates[i].position));
            }
        }
