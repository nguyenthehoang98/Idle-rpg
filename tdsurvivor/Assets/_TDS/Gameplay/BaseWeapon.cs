...
namespace _Game.GamePlay.Model
{
    public abstract class BaseWeapon : MonoBehaviour
    {
        ...
        [SerializeField] private float weaponFireDelay = 0.2f;
        [Header("Components")]
        ...
        protected virtual IEnumerator AutoAttackIE()
        {
            while (!IsAttacking)
            {
                ...
                int count = CurrentUpgradeData.projectileCountBonus + 1;

                entities.Clear();
                destinations.Clear();

                bool found = FindTarget(SkillData.findTarget, count, position);
                ...
                yield return RotateIE(position, destinations[0]);
                ...
                Debug.LogError($"P;{entities.Count} , {destinations.Count}, {count}");
        ...
        protected virtual void ExecuteAttack()
        {
            ...
            {
                ProjectileScaleBonus = CurrentUpgradeData.projectileScaleBonus,
                AttackRange = CurrentUpgradeData.attackRange,
                Attack = (1 + CurrentUpgradeData.projectileDamageMultiplier) * WeaponData.attack,
                ...
                ProjectilesPerShot = CurrentUpgradeData.projectilesPerShot,
                SpreadProjectileCount = CurrentUpgradeData.spreadProjectileCount,
                BonusPierceCount = CurrentUpgradeData.bonusPierceCount,
                ...
                ExecuteHealthPercent = CurrentUpgradeData.executeHealthPercent,
        ...
        private bool FindTarget(FindTargetType type, int totalQuery, Vector3 position)
        {
            Vector3 center = Vector3.zero;
            float radius = SkillData.attackRange + CurrentUpgradeData.attackRange;
            float sqrRadius = radius * radius;

            query.FindTarget(type, totalQuery, center, position, radius, (e, float2) =>
            {
            ...
            {
                foreach (var qe in result.Results)
                {
                    if (entities.Add(qe.Entity)) destinations.Add(qe.Position);
                }
            }
