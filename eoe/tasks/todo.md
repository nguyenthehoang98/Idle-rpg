...
**Current status:** Design docs đã commit; Energy Circuit core 5/5; item modifiers, board runtime, circuit tick runner, Hero Overdrive contract và ba Overdrive identities đã test/verify; hero combat engagement đã fix và verify.
...
## B-04 - Tạo 3 Overdrive identity `[P0]` `[x]`

- [x] DPS: tăng tốc đánh hoặc projectile.
- [x] AOE: tạo damage diện rộng hoặc chain.
- [x] Defense: shield/heal/damage reduction.
- [x] Mỗi identity có feedback khác nhau.

**Acceptance:** Có thể phân biệt ba Overdrive chỉ bằng gameplay/VFX.

**Verify:** `tools/verify-unity.ps1 -Mode all` — compile pass, 39 EditMode tests pass; DPS attack-speed modifier, AOE multi-target attack window và Defense shield có behavior tests.
