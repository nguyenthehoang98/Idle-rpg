...
namespace AnimationCurveManipulationTool {
    ...
    public class AnimationWindowStateBinding {
        ...
        public void ReverseKeys() {
            ...
            var curveGroupIndexes = new Dictionary<object, List<int>>();

            var tempKeyframes = new Keyframe[keys.Count];
            for (int i = 0; i < tempKeyframes.Length; i++) {
                tempKeyframes[i] = new Keyframe(keys[i].time, keys[i].value,
                                                keys[i].inTangent, keys[i].outTangent,
                                                keys[i].inWeight, keys[i].outWeight);
                if (!curveGroupIndexes.TryGetValue(keys[i].curve, out var groupKeyframeIndexes)) {
                    groupKeyframeIndexes = new List<int>();
                    curveGroupIndexes.Add(keys[i].curve, groupKeyframeIndexes);
                }
                groupKeyframeIndexes.Add(i);
            }

            foreach (var kvp in curveGroupIndexes) {
                var keyIndexes = kvp.Value;
                if (keyIndexes.Count < 2) continue;
                var curve = kvp.Key;

                for (int i=0; i<keyIndexes.Count; i++) {
                    var iReverse = keyIndexes.Count - i - 1;
                    var index = keyIndexes[i];
                    var reverseIndex = keyIndexes[iReverse];

                    keys[index].value = tempKeyframes[reverseIndex].value;
                    if (i != 0) {
                        keys[index].inTangent = -tempKeyframes[reverseIndex].outTangent;
                        keys[index].inWeight = tempKeyframes[reverseIndex].outWeight;
                    }
                    if (i != keyIndexes.Count - 1) {
                        keys[index].outTangent = -tempKeyframes[reverseIndex].inTangent;
                        keys[index].outWeight = tempKeyframes[reverseIndex].inWeight;
                    }
                    var curveBinding = new AnimationWindowCurveBinding(curve);
                    SaveCurve(curveBinding.clip, curveBinding, "reverse keyframe");
                }
            }
        ...
        public void FlipValues() {
            ...
            foreach (var kvp in curveGroupIndexes) {
                ...
                var selectedKeyframes = new List<AnimationWindowKeyframeBinding>();
                for (int i = 0; i < keyIndexes.Count; i++) {
                    selectedKeyframes.Add(keys[keyIndexes[i]]);
                }
                ...
                for (int i = 0; i < keyIndexes.Count; i++) {
                    ...
                    float delta = keys[index].value - minVal;
                    float reversedDelta = range - delta;
                    keys[index].value = minVal + reversedDelta;

                    if (i != 0) {
                        keys[index].inTangent *= -1;
                    }
                    if (i != keyIndexes.Count - 1) {
                        keys[index].outTangent *= -1;
                    }
                    var curveBinding = new AnimationWindowCurveBinding(curve);
                    SaveCurve(curveBinding.clip, curveBinding, "reverse keyframe");
                }
        ...
        private static float GetKeyframesValueRange(List<AnimationWindowKeyframeBinding> _keyframes, out float _minValue) {
